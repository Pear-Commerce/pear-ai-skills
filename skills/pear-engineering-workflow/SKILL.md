...
`db.sh` quoting: the helper takes SQL directly (`db.sh -e prod "<SQL>"`, joining trailing args with `$*`), and the SSM send-command/eval chain strips a shell-quoting layer, so single- or double-quoted SQL string literals arrive mangled — symptom: `ERROR 1064 ... near '%FOO%'`. Do not fight nested escaping. Use MySQL hex literals for string constants: `LIKE 0x2553484f505249544525` is `LIKE '%SHOPRITE%'`; generate with `echo -n '%SHOPRITE%' | xxd -p` and prefix `0x`. Table/column identifiers need no quotes, so hex literals cover the usual failures (LIKE filters, enum-name matches, string equality, date comparisons).
