...
- `devops/db.sh`: direct private DB helper. Requires an active split-tunnel AWS Client VPN — if it is not running, start `devops/vpn.sh` and leave it running; never use `--start-vpn`. Resolves the current private RDS/RDS Proxy target through VPC DNS, fetches `prod-db-10-2025` from Secrets Manager, and connects with TLS. Use `--dev`/`--analytics`, `--prod`, or `--read`; normal SQL quoting is preserved because there is no SSM/eval hop.
...
- `devops/db-tunnel.sh --dev|--prod`: compatibility localhost relay over Client VPN plus `socat`; it no longer uses an SSM bastion. Requires an active VPN — start `devops/vpn.sh` if it is not running; never `--start-vpn`.
- `devops/vpn.sh`: keeps the split-tunnel VPN open until Ctrl-C. This is the default and required way to hold private VPC access for `jsp.sh`, `db.sh`, `db-tunnel.sh`, and other local applications — run it in a second terminal while those tools are in use; agents without a second terminal can background it (`nohup devops/vpn.sh >/tmp/pear-vpn.log 2>&1 &`). Strict rule: if the VPN is not running, start `vpn.sh` — never fall back to `--start-vpn` on `db.sh`/`jsp.sh`/`db-tunnel.sh`, which couples the VPN lifetime to a single command's process.
