...
5. Treat direct resolver matches and graph candidates as different contracts:
   ...
   - `SRetailerItemData.itemId` and `UPCRetailerData.itemId` are direct retailer-specific ids for the retailer resolver/updater/PDP builder. `UPCRetailerData.instacartItemId` is the Instacart product id for Instacart fallback, Instacart batch updater, and Instacart list-scraper/list-check behavior. Do not diagnose an Instacart `instacartItemId` as if it were evidence that the direct retailer `itemId` path exists.
