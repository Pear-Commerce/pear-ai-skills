...
Do not graduate availability by adding a `UPCRetailerStaticAvailabilityRecomputer` subclass unless the live probe has proven fixed-status semantics for the supplied item id/UPC/PDP and fulfillment mode. That base class writes constant statuses and defaults to `AVAILABLE`, so it is not a harmless placeholder. A setup-only retailer or a retailer with only a PDP URL pattern should not get an availability updater yet; leave `UPCRetailerZipAvailabilityRecomputer.getInstance(enum)` null until a real availability route is proven.

For `getPdpUrl(...)`, first try to build the retailer PDP from `item.getOrCreateRetailerData(retailer.enumName).getItemId()` and known URL strings/patterns. If item id alone is not enough, have the resolver store the second stable value in `SRetailerItemData.secondaryId` and build from `itemId` plus `secondaryId`. Do not make `SRetailerItemData.url`, `UPCRetailerData.linkUrl`, or `SItemDataWrapper.getLink()` the primary strategy; those are fallbacks only when deterministic URL construction is impossible or unavailable. Do not accept full `http://` or `https://` URLs as normal `itemId` values, and do not add `if itemId startsWith http return itemId` logic; that blesses bad `UPCRetailerData` storage. Only keep URL-shaped item ids for an explicitly documented legacy/last-resort retailer, with a negative or exception-specific regression test.

For updater URL behavior, strongly prefer an abridged resolver-to-availability `@Script` as the default proof, not only a pure URL-helper unit test. Treat it as expected whenever PDP URL reconstruction, saved URL fallback removal, platform URL translation, `secondaryId` handoff behavior, or URL-shaped item-id rejection changes; if you omit it, say exactly why the shorter test still proves the production path. The script should prove the same data path production will use: create resolver-shaped `SRetailerItemData` with compact `itemId`, `secondaryId`, and a nonblank expected `url`, manually seed direct `UPCRetailerData`/`SItemDataWrapper` for `retailer.enumName`, construct a `UPCRetailerZipAvailability`, then call the availability updater URL path and assert the final PDP URL. Seed a URL-shaped `itemId` negative case when guarding against URL storage. Put a stale or wrong `PDPUrlSavedFromAvailabilityCheck` on the availability when the bug is "saved URL fallback" so the test proves the updater ignores it. Cover both current/new and legacy/platform-style retailers when the platform has multiple URL formats or historical behavior. Lazily create test `RetailPartner`, `UPC`, and direct `UPCRetailerData` rows only if the script needs ORM/Spring state; otherwise construct them in memory. Do not seed platform data, do not call `item.getPlatformDataIfPresent(retailer.availabilitySharedImagesAndIds)`, and do not let a passing test rely on shared-platform fallback when the production behavior should read direct retailer data.
...
## Strategy: Rendered in-context batch/availability

Some retailers render all product and store inventory in the backend and expose **no observable store-level inventory XHR**. The DevTools Network tab shows only document and asset loads — no JSON/fetch call carries stock, price, or store-specific data. This is common on Cloudflare/Turnstile, Forter, or session-bootstrapped storefronts (e.g. carrefour.fr). Standard network-sniffing finds no usable data endpoint, and the page refreshes/re-navigates so fast when a shopper changes store that the relevant calls vanish before they can be captured.

Do **not** conclude availability is impossible. The backend-rendered data is reachable; it just lives behind an in-browser session the Java process cannot reproduce statelessly. The "Rendered in-context batch" strategy gets at it without standing up local Playwright/headless browser infra on Pear boxes.

### When to use this strategy

Reach for it when **all** of these hold:

- DevTools (or a render-provider traffic dump) shows product/store data on the page but **no standalone inventory data request** — only document loads and assets.
- A live data endpoint exists (often a store-scoped `POST`/GraphQL that accepts many GTINs + store/facility ids) but it only succeeds **after** the site's anti-bot/session bootstrap (Cloudflare clearance, FRO/session cookies, Turnstile solve).
- Java replay of that endpoint statelessly is blocked: direct/proxy `POST` returns 403/challenge/app-shell, even with `.asChrome()` and browser profiles, because the clearance/session token is minted inside the browser.
- You can see (via a render provider's captured XHR log, frontend-bundle strings, or APK decompilation) that a batch-shaped endpoint exists and what its request/response shape is.

If instead the page *does* expose a clean inventory XHR that proxies can replay, use the normal Jurl/proxy path — this strategy is the heavier fallback.

### The strategy, step by step

1. **Sniff with the render provider, not local Chrome.** Request the PDP through a JS-rendering proxy (Scrapfly render/ASP, or ZenRows render) and inspect both the rendered HTML **and** the provider's captured traffic (`ScrapflyAPI.result.browser_data.xhr_call`, or the equivalent debug log). That traffic is the reliable observer when the page re-navigates and local DevTools loses the calls. Search the downloaded frontend bundle and rendered bootstrap link map for product-list, GTIN, search, mixable, recommendation, cart, or physical-store endpoints. If APK/XAPK decompilation is available, mine it for APIM/gateway base URLs, route fragments, DTO names, and parameter names — they often name the same batch endpoint the web app eventually calls.

2. **Identify the store-scoped batch endpoint.** Look for a single call that accepts **many GTINs and one or more store/facility ids** in one request (e.g. `POST /products` with a GTIN array + facilityServiceId). A batch endpoint is what makes this economical; a per-item/per-store call is usually too expensive to productionize. Record its method, URL, headers, body shape, and how store context is supplied.

3. **Execute the data call *inside* the rendered browser context.** The anti-bot clearance and session cookies are minted by rendering the PDP. The store-scoped data call only works *after* that bootstrap, so run it in the same rendered session via the render provider's `js_scenario` `execute`/`fetch` (Scrapfly) and return the internal API response as the scenario result. Parse that JSON in Java. You are not scraping the rendered DOM for stock text — you are using the rendered browser context as a privileged execution environment to fire the data API and bring back structured data. A rendered HTML 200 is **not** success; require the internal API response to echo the requested GTIN/store ids and expose definitive stock/price/buyability fields.

4. **Batch to amortize the expensive bootstrap.** One rendered session should resolve many products and stores, not one. Chunk at conservative limits and keep concurrency low — e.g. up to ~50 GTINs × ~5 stores per rendered session, single-digit store/UPC workers. The browser bootstrap is the expensive part; issuing multiple store/GTIN calls per scenario is what keeps cost sane. Document the chunk sizes you choose and revisit them against cost/latency.

5. **Resolve store ids inside the scenario, not via a separate migration.** When Pear's imported `Store.storeId` (legacy/marketing id) differs from the id the batch endpoint needs (e.g. a `facilityServiceId` like `7595-150-718`), resolve it *inside the rendered scenario* (e.g. `GET /store/id/{legacyId}`) rather than rewriting the store import or building a static id map. This keeps the store import focused on stable locator ids and avoids a brittle id-rewrite migration. Re-run the live store discovery/import to keep store ids current instead of shipping a static JSON.

6. **Map statuses carefully; never turn transient omissions into UNAVAILABLE.** Parse the location-dependent signal as `inStoreStatus` and any separate location-independent ecommerce signal as `shipToHomeStatus`; set `price` when exposed. A GTIN that is simply **missing** from a batch response should stay `UNKNOWN`, not `UNAVAILABLE`, because a transient omission is not evidence of zero stock — false negatives here pollute availability downstream. Reserve `UNAVAILABLE` for an explicit out-of-stock field for the requested store + item. Validate against **at least two real stores** and prove the store id changes the price/status before productionizing.

7. **Order the proxy ladder by cost, render-capable first.** The batch route needs in-page JS, so the ladder is render-capable proxies only (Scrapfly datacenter render → Scrapfly residential render for reliability). Put `UNBLOCKER` last for ordinary document fetches — it is the most expensive and **cannot execute the in-page JavaScript** the batch route requires, so it cannot satisfy this strategy. If ZenRows is temporarily out of credits, omit it and document why; revisit when credits are restored. Keep a separate cheap ladder (`STATIC`/datacenter) for the non-rendered document/URL routes so a single expensive render proxy does not become the only path for everything.

### Production shape

Graduate the proven route into an injectable retailer `@Service` client plus a `BatchAvailabilityUpdater`, not static feasibility methods. The client owns the rendered-session bootstrapping, the `js_scenario` batch call, store-id resolution, and response parsing; the batch updater drives the chunked `MAX_GTINS_PER_BATCH × MAX_STORES_PER_BATCH` work and the conservative worker counts. The single-item companion (`UPCRetailerZipAvailabilityRecomputer`) handles on-the-fly checks and online/ship-to-home. The `RetailPartner` migration sets `itemAvailabilityDependsOnZip = true` and `locationAgnosticShipToHome = false` for store-scoped `IN_STORE` (flip `locationAgnosticShipToHome = true` only when a separate, proven global ship-to-home signal exists). Set `markMissingStoreIdsUnavailable() = false` so missing stores stay `UNKNOWN`. Reference implementation: `com.pear.retailintegrations.carrefour` (`CarrefourClient`, `CarrefourBatchAvailabilityUpdater`, `CarrefourAvailabilityUpdater`).

### Cost guardrail

This strategy is deliberately more expensive than a stateless proxy route because every scan pays for a rendered browser session. Before committing to it, confirm the cheaper alternatives are genuinely blocked: a stateless `STATIC`/datacenter replay of the batch endpoint, `.asChrome()` + browser-profile retries, and any APK/mobile endpoint that can be replayed through a cheap proxy. Only when those are documented-fail should the rendered in-context batch become the production path. Maximize GTINs and stores per rendered session, keep the rollout conservative, and re-evaluate the ladder whenever proxy credit situations change.
...
When the shopper page visibly has product/store data but DevTools shows no useful standalone inventory request, use the render provider as the traffic observer instead of adding local Playwright. Request the page with Scrapfly render/ASP or ZenRows render and inspect both the rendered HTML and provider debug traffic (`ScrapflyAPI.result.browser_data.xhr_call`). Search the downloaded frontend bundle and the rendered bootstrap link map for product-list, GTIN, search, mixable, recommendation, cart, or physical-store endpoints. If a promising endpoint only works after Cloudflare/session bootstrap, execute it inside that same rendered context with Scrapfly `js_scenario` `execute`/`fetch`, return the response as the scenario result, and parse it in Java. Prefer batch-shaped calls that accept many GTINs and one or more store/facility IDs; validate at least two real stores and prove store-dependent price or status before productionizing. Keep expensive browser bootstraps low by issuing multiple store/GTIN calls per scenario, using small conservative chunks and low concurrency. A rendered HTML 200 without captured data is not success; require the internal API response to echo the requested IDs and expose definitive buyability/price fields. This paragraph is the general case of the named "Rendered in-context batch" strategy documented above; use that section when you hit the specific no-data-XHR + Cloudflare/session-bootstrap pattern.
