...
- Keep the AWS Client VPN active while using the private JSP URL. The default workflow: run `devops/vpn.sh` in a second terminal (it holds the VPN until Ctrl-C), then run `devops/jsp.sh`, which verifies the VPN is up, prints the private `http://<instance-private-ip>:8080/...` URL plus the compile-check output, and exits — it does not hold the VPN. Alternatively pass `--start-vpn` and `jsp.sh` itself opens the VPN and parks until Ctrl-C. Always open the printed URL in a real browser with no query parameters, and do not click `Run` unless the user approves it.
- On a workstation without the guarded VPN helper, the first `jsp.sh` invocation runs one-time setup. If a non-interactive Codex terminal cannot satisfy the macOS sudo prompt, relay the script's exact `./devops/setup-client-vpn.sh` command to the user and wait for them to run it in their own terminal. Do not bypass the helper, add broad sudoers permissions, or fall back to public JSP access.
- Treat `devops/jsp.sh` output as the compile/load authority. It copies the JSP into the live container and curls the signed no-parameter path over localhost through non-interactive SSM Run Command; it must print the captured remote stdout/stderr locally and must not use `start-session`. Read that output for the HTTP response and JSP compilation errors such as `Unable to compile class for JSP`. Do not add `pear_debug` to this request: that is an employee-only diagnostic control and is not part of JSP compilation or capability authorization. Do not create ad hoc shell scripts, `ec2-exec` wrappers, docker-curl helpers, or alternate compile-check paths for a JSP you just loaded. If the `jsp.sh` output is truncated, noisy, or inconclusive, rerun `devops/jsp.sh` on the JSP and inspect that output instead of inventing a second script.
- Run the compile/deploy preview without `--single` when the no-parameter path is side-effect-free; this keeps the JSP available across the environment even though the printed private URL pins the browser preview to one selected instance. Use `--single` only when invoking side effects from the helper path, which should usually be avoided in favor of the browser `Run` button.
- After browser verification, failure, or abandoning the task, close the VPN session when no other terminal needs it: Ctrl-C the `vpn.sh` terminal (or the `jsp.sh --start-vpn` session) and confirm `Closing AWS Client VPN...`. Do not leave the VPN process running unattended.
...
- For long-running migrations, broad loops, resolver/import batches, or anything likely to outlive a comfortable browser request, emit progress to both the response and server logs. Use a stable `LOG` prefix plus `System.out.println(...)` or `Resources.global().logger.info(...)` at run start, each major phase, periodic item milestones, errors, and run finish so `devops/logs.sh` can track the job even if the browser disconnects. Include enough counters and target IDs to reconstruct status from DB/logs without the final HTML report.
- For large migrations, add a bounded monitoring plan before Run: log every vendor/phase and every N items, keep writes idempotent so a rerun can continue safely, and prefer a TEST logging smoke when practical. On multi-instance PROD, remember that a single `devops/logs.sh` stream may attach to the wrong backend; search all running instances or use another environment/log source when confirming the `LOG` prefix.
...
By default `devops/jsp.sh` exits when the run is done: after the URL, S3 archive, and every selected instance's deploy/compile-check output, the process exits 0 on success and non-zero on failure. Completion detection is process exit; agents and scripts may chain commands normally — `jsp.sh ... && next-cmd` works. If the VPN is not active, it fails fast with instructions before signing or uploading anything; start `devops/vpn.sh` in a second terminal (or rerun with `--start-vpn`) and retry.

The URL line prints before the compile-check output; it is not a completion signal. Read the captured output for the localhost HTTP response and JSP compilation errors such as `Unable to compile class for JSP`.

With `--start-vpn`, `jsp.sh` instead parks holding AWS Client VPN open until Ctrl-C, and completion must be detected by output, not process exit. The fixed line `AWS Client VPN is active for the private JSP URL. Press Ctrl-C to close it.` is the end-of-output sentinel: it prints only after every selected instance's deploy/compile-check output is final, and on a failed deploy/exec step the script exits non-zero without ever printing it. Then SIGINT the process, confirm `Closing AWS Client VPN...`, and parse the captured log.

Background-run pattern for agents (default path):
...
The private-IP URL printed by `devops/jsp.sh` does not go through Cloudflare. Do not add the trusted-edge header to that URL.
...
- `devops/jsp.sh` uses AWS KMS to generate a one-hour HMAC capability embedded in the temporary JSP filename, uploads the source to `s3://assets.pearcommerce.com/jsp-log/`, copies it into the live container over SSM, curls the same signed path over localhost, verifies the EB-persisted `pear-jsp-vpn-access` group, requires an active AWS Client VPN (`devops/vpn.sh`, or its own `--start-vpn`), and prints a private-IP browser URL. `devops/jspx` uses the same capability helper and SSM—not public-IP SSH/rsync—for retained-JSP compatibility execution. Unknown JSPs without a valid unexpired capability are rejected even over localhost; retained repo JSPs keep their explicit public/internal/employee policy.
- The printed private URL is a one-hour bearer capability and is reachable only while the workstation VPN session remains active. Do not paste it into Slack, tickets, logs, or other durable/shared locations. Open it directly in the browser, and rerun `jsp.sh` when it expires rather than trying to edit or reuse the filename.
...
5. Make sure the AWS Client VPN is active — run `devops/vpn.sh` in a second terminal and leave it running (or plan to pass `--start-vpn`). Then deploy/compile with an explicit env and no `--single`, relying on the no-parameter side-effect-free preview:

```bash
devops/jsp.sh -j /tmp/descriptive-prod-read.jsp -e PROD
```

6. Read the `devops/jsp.sh` output; the command exits when the run is done. If it reports the VPN is not active, start `devops/vpn.sh` (or rerun with `--start-vpn`) and retry. If it reports that one-time macOS administrator setup is required, relay the exact setup command and wait for the user to complete it; no JSP has been signed or uploaded yet. If output shows a JSP compile error, patch the JSP and rerun `devops/jsp.sh`; do not create a separate shell script or alternate `ec2-exec`/docker-curl path to test the same JSP.
7. While the VPN remains active in its own terminal, open the printed private-IP URL in a real browser with no query parameters. Confirm the preview page shows the plan and `Run` button. The `jsp.sh` helper output is the compile/load check, but it does not replace opening the browser preview.
...
9. Capture and summarize the run report, timings, errors, private URL, S3 source key, and follow-up verification when useful.
10. When the task is done (success, browser failure, compile failure, cancellation, or any other exit path), Ctrl-C the `vpn.sh` terminal — or the `jsp.sh --start-vpn` session — and confirm `Closing AWS Client VPN...`. Leave the VPN open only if another task still needs it.
