...
- Keep the AWS Client VPN active while using the private JSP URL. The workflow: run `devops/vpn.sh` in a second terminal and leave it running (it holds the VPN until Ctrl-C; agents without a second terminal can background it, e.g. `nohup devops/vpn.sh >/tmp/pear-vpn.log 2>&1 &`), then run `devops/jsp.sh`, which verifies the VPN is up, prints the private `http://<instance-private-ip>:8080/...` URL plus the compile-check output, and exits — it does not hold the VPN. Strict rule: if the VPN is not running, start `vpn.sh` — never pass `--start-vpn`, which parks `jsp.sh` until Ctrl-C and couples the VPN lifetime to one command. Always open the printed URL in a real browser with no query parameters, and do not click `Run` unless the user approves it.
...
- After browser verification, failure, or abandoning the task, close the VPN session when no other terminal needs it: Ctrl-C (or kill) the `vpn.sh` session and confirm `Closing AWS Client VPN...`. Do not leave the VPN process running unattended.
...
By default `devops/jsp.sh` exits when the run is done: after the URL, S3 archive, and every selected instance's deploy/compile-check output, the process exits 0 on success and non-zero on failure. Completion detection is process exit; agents and scripts may chain commands normally — `jsp.sh ... && next-cmd` works. If the VPN is not active, it fails fast with instructions before signing or uploading anything; start `devops/vpn.sh` (never rerun with `--start-vpn`) and retry.
...
Do not use `--start-vpn`: it parks `jsp.sh` holding the VPN until Ctrl-C, couples the VPN lifetime to a single command, and breaks process-exit completion detection. The VPN always belongs to a separate `vpn.sh` session, so `jsp.sh` itself always exits when the run is done.
...
- `devops/jsp.sh` uses AWS KMS to generate a one-hour HMAC capability embedded in the temporary JSP filename, uploads the source to `s3://assets.pearcommerce.com/jsp-log/`, copies it into the live container over SSM, curls the same signed path over localhost, verifies the EB-persisted `pear-jsp-vpn-access` group, requires an active AWS Client VPN (`devops/vpn.sh` — start it if it is not running; never `--start-vpn`), and prints a private-IP browser URL. `devops/jspx` uses the same capability helper and SSM—not public-IP SSH/rsync—for retained-JSP compatibility execution. Unknown JSPs without a valid unexpired capability are rejected even over localhost; retained repo JSPs keep their explicit public/internal/employee policy.
...
10. When the task is done (success, browser failure, compile failure, cancellation, or any other exit path), Ctrl-C (or kill) the `vpn.sh` session and confirm `Closing AWS Client VPN...`. Leave the VPN open only if another task still needs it.
