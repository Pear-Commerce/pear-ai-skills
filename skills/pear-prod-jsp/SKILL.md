...
- Run `devops/jsp.sh` in a persistent PTY/terminal session. It opens AWS Client VPN, prints a private `http://<instance-private-ip>:8080/...` JSP URL, and intentionally remains running until Ctrl-C. Keep that session alive while using the JSP, always open the printed URL in a real browser with no query parameters, and do not click `Run` unless the user approves it.
...
- Always close the persistent `jsp.sh` session with Ctrl-C after browser verification, on failure, or when abandoning the task. Confirm the output includes `Closing AWS Client VPN...`; do not leave the VPN process or terminal session running.
...
- `devops/jsp.sh` uses AWS KMS to generate a one-hour HMAC capability embedded in the temporary JSP filename, uploads the source to `s3://assets.pearcommerce.com/jsp-log/`, copies it into the live container over SSM, curls the same signed path over localhost, verifies the EB-persisted `pear-jsp-vpn-access` group, opens AWS Client VPN, and prints a private-IP browser URL. `devops/jspx` uses the same capability helper and SSM—not public-IP SSH/rsync—for retained-JSP compatibility execution. Unknown JSPs without a valid unexpired capability are rejected even over localhost; retained repo JSPs keep their explicit public/internal/employee policy.
...
5. Deploy/compile with an explicit env and no `--single`, relying on the no-parameter side-effect-free preview. Start the command in a persistent PTY/terminal session; do not wait for it to exit because it intentionally holds AWS Client VPN open:

```bash
devops/jsp.sh -j /tmp/descriptive-prod-read.jsp -e PROD
```

6. Read live output from the still-running `devops/jsp.sh` session. If it reports that one-time macOS administrator setup is required, relay the exact setup command and wait for the user to complete it; no JSP has been signed or uploaded yet. Otherwise, wait for the localhost compile/load response and the message that AWS Client VPN is active. If output shows a JSP compile error, send Ctrl-C, patch the JSP, and rerun `devops/jsp.sh`; do not create a separate shell script or alternate `ec2-exec`/docker-curl path to test the same JSP.
7. While that session remains active, open the printed private-IP URL in a real browser with no query parameters. Confirm the preview page shows the plan and `Run` button. The `jsp.sh` helper output is the compile/load check, but it does not replace opening the browser preview.
...
10. In a cleanup/finally step, send Ctrl-C to the persistent `jsp.sh` session and confirm `Closing AWS Client VPN...`. Perform this cleanup after success, browser failure, compile failure, cancellation, or any other exit path.
