...
Run one orchestrator maintenance cycle for the configured remote Codex worker host: self-refresh this orchestrator automation if stale, ensure slot threads exist, ask slot threads to self-refresh their own automations, publish host heartbeat, print REMOTE_CODEX_FLEET_STATUS_JSON from host and slot heartbeats, print REMOTE_CODEX_TASK_LOG_JSON from slot task events, and repair drift. Do not execute queue jobs in the orchestrator.
```
...
The orchestrator final response for every maintenance wake should include a machine-readable fleet status block before the task log block:
...
The orchestrator final response for every maintenance wake should include a machine-readable task log block after the fleet status block:
...
- Repair stale automation prompts whose `remoteCodexBundleVersion` is missing or not `2026-06-08.14`.
