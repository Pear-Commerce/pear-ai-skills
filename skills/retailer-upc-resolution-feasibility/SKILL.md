...
For URD fields, `itemId` is the direct retailer-specific id that should feed the retailer's resolver, availability updater, and PDP URL builder. `instacartItemId` is reserved for the Instacart product id that feeds Instacart fallback, Instacart batch availability, and Instacart list-scraper checks. Do not store an Instacart product id in a retailer-specific `itemId` unless the retailer itself is the Instacart integration; keep direct retailer evidence and Instacart platform evidence distinguishable in resolver outputs and notes.
