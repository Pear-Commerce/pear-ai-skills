...
- Do not leave a direct third-party logo URL, local file path, data URL, or runtime migration upload as the production value. If local credentials, CI, or the workstation cannot upload, load `$pear-prod-jsp` and use a temporary `devops/jsp.sh` preview/Run-button workflow on a live box to perform the upload through Pear infrastructure, then hardcode the resulting Pear asset URL. Keep its private-IP preview VPN session open through verification and Ctrl-C it afterward.
