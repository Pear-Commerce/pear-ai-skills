...
Never store a full `http://` or `https://` PDP URL in `SRetailerItemData.itemId` or `UPCRetailerData.itemId` during normal resolver or integration work. `itemId` should be the compact retailer product id, SKU, slug, variant id, or path fragment needed for deterministic reconstruction. Put the full PDP in `SRetailerItemData.url` as evidence/fallback, and use `secondaryId` for the second stable value when `itemId` alone is not enough. A URL-shaped `itemId` is allowed only as a deliberately documented last resort when the retailer exposes no stable id or id-plus-secondary-id pattern; that exception must be called out in the PR and covered by a test so it cannot become the default table shape.
